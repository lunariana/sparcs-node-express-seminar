...
const AccountPage = () => {
  ...
  const getAccountInformation = () => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/getInfo', { username: SUsername, password: SPassword });
  ...
  const performTransaction = ( amount: number | '' ) => {
    const asyncFun = async() => {
      ...
      const { data } = await axios.post<IAPIResponse>(SAPIBase + '/account/transaction', { username: SUsername, password: SPassword, amount: amount });
      setNTransaction(0);
      if (!data.success) {
  ...
  return (
    <div className={"account"}>
      ...
      <div className={"account-token-input"}>
        ...
        Enter Username: <input type={"text"} value={SUsername} onChange={e => setSUsername(e.target.value)}/><br></br>
        Enter Password: <input type={"password"} value={SPassword} onChange={e => setSPassword(e.target.value)}/><br></br>
